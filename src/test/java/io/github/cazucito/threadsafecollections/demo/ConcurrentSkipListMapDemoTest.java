...
/**
 * Tests para ConcurrentSkipListMapDemo.
 */
@DisplayName("ConcurrentSkipListMapDemo Tests")
class ConcurrentSkipListMapDemoTest {

    private final ConcurrentSkipListMapDemo demo = new ConcurrentSkipListMapDemo();

    @Test
    @DisplayName("Demo has correct id")
    void demoHasCorrectId() {
        assertEquals("concurrent-skip-list-map", demo.id());
    }
    ...
    void demoHasNonEmptyTitle() {
        ...
        assertTrue(demo.title().toLowerCase().contains("skip") || demo.title().toLowerCase().contains("map"));
    }
    ...
    void runReturnsValidDemoResult() {
        ...
        assertEquals("concurrent-skip-list-map", result.demoId());
