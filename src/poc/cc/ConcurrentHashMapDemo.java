 ...
 * not on its synchronization details.
 * 
 ...
 * happens-before relation with any (non-null) retrieval for that key reporting
 * the updated value.) For aggregate operations such as putAll and clear,
 ...
 * status methods including size, isEmpty, and containsValue are typically
 ...
 * size estimate as an optional initialCapacity constructor argument. An
 * additional optional loadFactor constructor argument provides a further means
 * of customizing initial table capacity by specifying the table density to be
 * used in calculating the amount of space to allocate for the given number of
 * elements. Also, for compatibility with previous versions of this class,
 * constructors may optionally specify an expected concurrencyLevel as an
 * additional hint for internal sizing. Note that using many keys with exactly
 * the same hashCode() is a sure way to slow down performance of any hash table.
 * To ameliorate impact, when keys are Comparable, this class may use comparison
 * order among keys to help break ties.
 * 
 * A Set projection of a ConcurrentHashMap may be created (using newKeySet() or
 * newKeySet(int)), or viewed (using keySet(Object) when only keys are of
 * interest, and the mapped values are (perhaps transiently) not used or all
 * take the same mapping value.
 * 
 * A ConcurrentHashMap can be used as scalable frequency map (a form of
 * histogram or multiset) by using LongAdder values and initializing via
 * computeIfAbsent. For example, to add a count to a
 * ConcurrentHashMap<String,LongAdder> freqs, you can use
 ...
 * that, unlike most Stream methods, are designed to be safely, and often
 ...
 * and/or return values. Because the elements of a ConcurrentHashMap are not
 ...
 * actions, should ideally be side-effect-free. Bulk operations on Map.Entry
 * objects do not support method setValue.
 * 
 * - forEach: Perform a given action on each element. A variant form applies a
 * given transformation on each element before performing the action. - search:
 * Return the first available non-null result of applying a given function on
 * each element; skipping further search when a result is found. - reduce:
 * Accumulate each element. The supplied reduction function cannot rely on
 * ordering (more formally, it should be both associative and commutative).
 * There are five variants: - Plain reductions. (There is not a form of this
 * method for (key, value) function arguments since there is no corresponding
 * return type.) - Mapped reductions that accumulate the results of a given
 * function applied to each element. - Reductions to scalar doubles, longs, and
 * ints, using a given basis value.
 * 
 * These bulk operations accept a parallelismThreshold argument. Methods proceed
 * sequentially if the current map size is estimated to be less than the given
 * threshold. Using a value of Long.MAX_VALUE suppresses all parallelism. Using
 * a value of 1 results in maximal parallelism by partitioning into enough
 * subtasks to fully utilize the ForkJoinPool.commonPool() that is used for all
 ...
 * ConcurrentHashMap: Any non-null result returned from get(key) and related
 * access methods bears a happens-before relation with the associated insertion
 * or update. The result of any bulk operation reflects the composition of these
 * per-element relations (but is not necessarily atomic with respect to the map
 * as a whole unless it is somehow known to be quiescent). Conversely, because
 * keys and values in the map are never null, null serves as a reliable atomic
 ...
 * with any other value, returns that other value (more formally, it should be
 * the identity element for the reduction). Most common reductions have these
...
public class ConcurrentHashMapDemo {
	...
	public static void testConstructores() {
		...
		// ConcurrentHashMap() => Creates a new, empty map with the default initial
		// table size (16).
		ConcurrentMap<Integer, String> chm1 = new ConcurrentHashMap<>();
		chm1.put(1, "UNO");
		chm1.put(2, "DOS");
		chm1.put(3, "TRES");
		Impresor.muestraEnConsola(TipoMensajes.DEPURACION, chm1.toString());
		// ConcurrentHashMap(int initialCapacity) => Creates a new, empty map with an
		// initial table size accommodating the specified number of elements without the
		// need to dynamically resize.
		ConcurrentMap<Integer, String> chm2 = new ConcurrentHashMap<>(3);
		chm2.put(1, "UNO");
		chm2.put(2, "DOS");
		chm2.put(3, "TRES");
		Impresor.muestraEnConsola(TipoMensajes.DEPURACION, chm2.toString());
		// ConcurrentHashMap(int initialCapacity, float loadFactor) => Creates a new,
		// empty map with an initial table size based on the given number of elements
		// (initialCapacity) and initial table density (loadFactor).
		ConcurrentMap<Integer, String> chm3 = new ConcurrentHashMap<>(3, 0.75F);
		chm3.put(1, "UNO");
		chm3.put(2, "DOS");
		chm3.put(3, "TRES");
		Impresor.muestraEnConsola(TipoMensajes.DEPURACION, chm3.toString());
		// ConcurrentHashMap(int initialCapacity, float loadFactor, int
		// concurrencyLevel) => Creates a new, empty map with an initial table size
		// based on the given number of elements (initialCapacity), table density
		// (loadFactor), and number of concurrently updating threads (concurrencyLevel).
		ConcurrentMap<Integer, String> chm4 = new ConcurrentHashMap<>(3, 0.75F, 4);
		chm4.put(1, "UNO");
		chm4.put(2, "DOS");
		chm4.put(3, "TRES");
		Impresor.muestraEnConsola(TipoMensajes.DEPURACION, chm4.toString());
		// ConcurrentHashMap(Map<? extends K,? extends V> m) => Creates a new map with
		// the same mappings as the given map.
		Map<Integer, String> datos1 = new HashMap<>();
		datos1.put(1, "UNO");
		datos1.put(2, "DOS");
		datos1.put(3, "TRES");
		ConcurrentMap<Integer, String> chm5 = new ConcurrentHashMap<>(datos1);
		Impresor.muestraEnConsola(TipoMensajes.DEPURACION, chm5.toString());

	}

	/**
	 * Prueba ConcurrentHashMap
	 */
	public static void testConcurrentHashMap() {
		Impresor.muestraEnConsola(TipoMensajes.SUBTITULO, "HashMap - FAIL-FAST");
		// HashMap
		...
		// ConcurrentHashMap
		Impresor.muestraEnConsola(TipoMensajes.SUBTITULO, "ConcurrentHashMap - FAIL-SAFE");
		ConcurrentMap<Integer, String> chm = new ConcurrentHashMap<>();
		chm.put(1, "UNO");
		chm.put(2, "DOS");
		chm.put(3, "TRES");
		ExecutorService hilos2 = Executors.newCachedThreadPool();
		hilos2.execute(new AdicionadorAMapaNoSincronizado(chm, 4, "CUATRO"));
		hilos2.shutdown();
